...
public class BookDao {
	 ...
	 * 
	 * @param bid 图书的ID
	 * @param adminbean 当前登录用户的信息
	 */
	public void borrowBook(int bid, AdminBean adminbean) {
	    // 获取图书信息
	    BookBean bookbean = new BookBean();
	    bookbean = this.get_BookInfo(bid); // 调用方法获取图书信息

	    // 获取当前日期作为借阅开始时间
	    Calendar c = Calendar.getInstance();
	    int year = c.get(Calendar.YEAR);
	    int month = c.get(Calendar.MONTH) + 1; // 月份从0开始，所以加1
	    int day = c.get(Calendar.DATE);
	    // 格式化借阅开始时间
	    String begintime = "" + year + "-" + month + "-" + day;

	    // 计算借阅结束时间（这里简单地将结束时间设为开始时间的下一个月的同一天）
	    month = month + 1; // 月份加1
	    // 格式化借阅结束时间
